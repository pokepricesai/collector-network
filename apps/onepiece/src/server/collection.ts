// OP collection CRUD + valuation composition.
//
...
async function fetchCardsById(
  ...
  for (let i = 0; i < cardIds.length; i += IN_BATCH) {
    ...
    if (error) throw new Error(`[onepiece/collection] cards: ${error.message}`);
...
// Delete every OP row owned by the caller. Used by the danger-zone
// "delete OnePiecePrices data" flow before auth-metadata cleanup +
